...
from sqlalchemy import ARRAY, INTEGER, Column, DateTime
...
class Shipment(SQLModel, table=True):
    ...
    id: UUID = Field(sa_column=Column(postgresql.UUID, default=uuid4, primary_key=True))
    created_at: datetime = Field(sa_column=Column(postgresql.TIMESTAMP,
...
class ServiceableLocation(SQLModel, table=True):
    ...
    partner_id: UUID = Field(foreign_key="delivery_partner.id",
                             primary_key=True)
    location_id: int = Field(foreign_key="location.zip_code",
                              primary_key=True)
